...
public class Det_Settings {
    ...
    public static void setSettings(det_methods second, det_methods thirst, int border){
        ...
        settings[1] = second;
        settings[2] = thirst;
                      Det_Settings.border = border;
    }
    public static void setBorder(int border){
        Det_Settings.border = border;
    }
    public static det_methods get_det_method( int n) throws MRV.MATRIX_DIMENSION_MISSMATCH {
        if (n>0){
            if (n >= border) return det_methods.TRIANGLE;
            else {
